...
#define QLScreenWidth  [UIScreen mainScreen].bounds.size.width
#define QLScreenHeight [UIScreen mainScreen].bounds.size.height
...
- (void)viewDidLoad {
    [super viewDidLoad];
//    self.view.backgroundColor = [UIColor colorWithC]
...
- (void)loadDefaultSetting {
    ...
    // 给SCrollView添加图片控件
    NSArray *arrImageNames = @[@"UIV2_InterestCard_Background_green", @"UIV2_InterestCard_Background_gary"];
...
- (void)loadEnjoyButton:(UIImageView *)imageView {
    ...
    [btnEnjoy setTitle:@"开始体验" forState:UIControlStateNormal];
    [btnEnjoy setTitleColor:[UIColor orangeColor] forState:UIControlStateNormal];
    btnEnjoy.titleLabel.font = [UIFont italicSystemFontOfSize:18];
    [self.scrollView addSubview:btnEnjoy];

    CGFloat width = 100;
    CGFloat X = (QLScreenWidth - width) * 0.5 + CGRectGetMinX(imageView.frame);
    CGFloat Y = QLScreenHeight - 130;
    btnEnjoy.frame = CGRectMake(X, Y, width, 30);
