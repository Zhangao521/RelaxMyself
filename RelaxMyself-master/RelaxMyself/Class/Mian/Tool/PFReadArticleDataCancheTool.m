...
@implementation PFReadArticleDataCancheTool
...
+ (void)saveDataWithArr:(NSArray *)arr idstr:(NSString *)idstr
{
    ...
    for (PFArticleModel *model in arr) {
    ...
    [_queue inDatabase:^(FMDatabase *db) {
        ...
        if (result) {
            while ([result next]) {
                ...
                PFArticleModel *model = [NSKeyedUnarchiver unarchiveObjectWithData:data];
                [arr addObject:model];
            }
        }
    }];
